...
//defines the compact representation of a row:
...
/*
get and set that value in a node
*/
...
//converts to and from the compact representation.
...
//get_page() method has the logic for handling a cache miss
...
//Advances the cursor in current table structure by incrementing the row number. 
...
//do_meta_command is just a wrapper for existing functionality 
//that leaves room for more commands:
...
// inserting a key/value pair into a leaf node.
